  ...
  const renderForm = () => {
    ...
    return (
      <form className="flex flex-col items-center" onSubmit={handleFormSubmit}>
        ...
        <div className="flex justify-center mb-3 flex-wrap sm:w-max w-96">
          {icons.map((icon) => {
            return (
              <div key={icon.id} className="mx-1">
                <img src={icon.src} alt={icon.label} name="icon" className={`hover:opacity-50 m-1 h-14 rounded-full ${icon.active ? 'bg-white' : ''}`} onClick={() => profileIconSelect(icon.id)}/>
              </div>
            )
          })}
        </div>
  ...
  return (
    <main className="bg-darkest text-gray-300 flex flex-col items-center justify-center mt-32 font-body">
      ...
      <div>
        ...
        {error && error.message.includes("email") && (
          <div className="m-8 rounded-md bg-black-700 shadow-[5px_2px_25px_5px_#6e91b8b6] p-4">
            <div className="flex">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-body text-white-800">Invalid signup info</h3>
        ...
        {error && error.message.includes("icon") && (
          <div className="m-8 rounded-md bg-black-700 shadow-[5px_2px_25px_5px_#6e91b8b6] p-4">
            <div className="flex">
              <div className="flex-shrink-0">
                <svg className="h-5 w-5 text-red-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-body text-white-800">Please select a profile icon.</h3>
              </div>
            </div>
          </div>
        )}
